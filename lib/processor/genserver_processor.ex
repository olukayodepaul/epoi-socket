defmodule Application.Processor do
  ...
  @impl true
  def handle_cast(:start_monitor, %{eid: _eid, user_id: user_id} = state) do
    ConnectSupervisorMonitor.start_supervisor_monitor(user_id)
    {:noreply, state}
  end

  def handle_cast(:stop_genserver_session , %{eid: eid, user_id: user_id} = state) do
    IO.inspect("GenServer Terminated Pass 4")
    StartSupervisorMonitor.terminate_monitor(%{eid: eid, user_id: user_id})
    {:stop, :normal, state}
  end
